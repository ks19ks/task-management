...
RSpec.describe Task, type: :model do
  ...
  describe 'バリデーションのテスト' do
    context 'タスクのタイトルが空の場合' do
      it 'バリデーションにひっかかる' do
        task = Task.new(title: '', description: '失敗テスト', user: user_a)
        expect(task).not_to be_valid
      end
    end
    context 'タスクの詳細が空の場合' do
      it 'バリデーションにひっかかる' do
        task = Task.new(title: 'タスクタイトル', description: '', user: user_a)
        expect(task).not_to be_valid
      end
    end
    context 'タスクのタイトルと詳細に内容が記載されている場合' do
      it 'バリデーションが通る' do
        task = Task.new(title: 'タスクタイトル', description: 'タスク説明', user: user_a)
  ...
  describe "検索機能" do
    let!(:task) { FactoryBot.create(:task, title: 'this is task', status: '完了', user: user_a) }
    let!(:task2) { FactoryBot.create(:task2, title: 'sample', status: '未着手', user: user_a) }
